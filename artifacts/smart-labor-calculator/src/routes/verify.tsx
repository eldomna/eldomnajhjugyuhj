...
import { ShieldCheck, ShieldAlert, Search, FileText, Loader2 } from "lucide-react";
...
export const Route = createFileRoute("/verify")({
  ...
  head: () => ({
    meta: [
      { title: "التحقق من الملفات • حاسبة العمال الذكية" },
      { name: "description", content: "تحقق من صحة تقارير حساب الحقوق العمالية الصادرة عن المنصة برقم الملف التسلسلي." },
    ],
  }),
...
function VerifyPage() {
  ...
  const submit = async (e?: React.FormEvent) => {
    ...
    // local fallback (offline-issued docs from this device)
      ...
      <main className="flex-1">
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-2xl">
              <div className="inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur px-3 py-1 text-xs font-medium mb-4">
                <ShieldCheck className="h-3.5 w-3.5" /> صفحة التحقق من صحة الملفات
              </div>
              <h1 className="font-display text-3xl sm:text-4xl font-extrabold">تحقّق من رسمية الملف</h1>
              <p className="mt-3 opacity-90 text-sm sm:text-base leading-relaxed">
                أدخل الرقم التسلسلي للملف (مثال: <code className="font-mono">YML-2026-00001</code>) للتحقق من صدوره عن المنصة.
                لا يتطلب تسجيل دخول.
              </p>
            </div>
          </div>
        ...
        <section className="container mx-auto px-4 -mt-8 max-w-2xl pb-16">
                  ...
                  <div className="flex items-center gap-3 text-primary mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-primary/80">
                        {status.source === "local" ? "محقق من السجل المحلي على هذا الجهاز" : "محقق من سجل المنصة"}
                      </p>
                    </div>
                  </div>
          ...
          <p className="text-center text-xs text-muted-foreground mt-6">
            تحتاج إلى إصدار تقرير؟ <Link to="/calculator" className="text-primary font-semibold">افتح الحاسبة</Link>
          </p>
        </section>
      </main>
    </div>
